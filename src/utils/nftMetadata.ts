...
export function createCheckpointMetadata(
  ...
  return {
    ...
    description: `Proof of visit to ${
      params.checkpointName
    } on ${collectedDate.toLocaleDateString()} at ${collectedDate.toLocaleTimeString()}. This NFT serves as immutable proof of your location-based achievement.`,
    ...
    attributes: [
      ...
      {
        trait_type: "Location",
        value: `${params.location.lat.toFixed(
          6
        )}, ${params.location.lng.toFixed(6)}`,
      },
      {
        trait_type: "Verified Location",
        value: `${params.verifiedLocation.lat.toFixed(
          6
        )}, ${params.verifiedLocation.lng.toFixed(6)}`,
      },
      {
